...
void lvl0Default() {
  ...
  CurMen_Time = millis();
  if ((CurMen_Time - LoopMen_Time) % 100 == 0) { //only update lvl0 every 100ms
    lcdUpToDate = false;
    //Serial.println("Menu Updated on 100ms timer");
  }
  if (lcdUpToDate != true) {
    lcdUpToDate = true;
    ...
    if ((CurMen_Time > (LoopMen_Time + 6000)) && (CurMen_Time <= (LoopMen_Time + 8000))) {
      ...
      if ((CurMen_Time > (LoopMen_Time+8000)) && (CurMen_Time <= (LoopMen_Time+10000))){
        ...
        lcd.setCursor(2,1);
        lcd.print((millis()-fcast_Time)/(1000.0*60.0));
        lcd.print("min ago    ");
      }
  ...
  if (lcdUpToDate != true) {
    ...
    switch (lvl2) {
      case 0: //Calibrate
        if ((purgeState != 1) && (waterState == false)) {
          lcd.setCursor(0, 0);
          lcd.print("Starting Flow");
          lcd.setCursor(0, 1);
          lcd.print("Calibration");
          waterManual();
          CalFlow = true;
        }
        else if ((purgeState == 1) || (waterState == true)) {
          lcd.setCursor(0, 0);
  ...
  if (lcdUpToDate != true) {
    ...
    switch (lvl2) {
      ...
      case 1: //Calibrate
        ...
        lcd.print("Calibration:");
        lcd.setCursor(0, 1);
        lcd.print("View Ideal Flows");
        break;
      case 2: //Calibrate
        lcd.setCursor(0, 0);
        lcd.print("Calibration:");
        lcd.setCursor(1, 1);
        lcd.print("Calibrate zTemp");
        break;
    }
  }
}
