...
int waterLimit = 0.5; //rain in inches to prevent watering
...
void Sprinkle(){
  ...
  CurWat_Time = millis();
  if((CurWat_Time >= LoopWat_Time) && (CurWat_Time <= (LoopWat_Time + 5000))){ //Water Zone 1
    if(WTmp == 1){
  ...
  else if((CurWat_Time > (LoopWat_Time+5000)) && (CurWat_Time <= (LoopWat_Time+10000))){ //Water Zone 2
    if(WTmp == 2){
  ...
  else if((CurWat_Time > (LoopWat_Time+10000)) && (CurWat_Time <= (LoopWat_Time+15000))){ //Water Zone 3
    if(WTmp == 3){
  ...
  else if((CurWat_Time > (LoopWat_Time+15000)) && (CurWat_Time <= (LoopWat_Time+20000))){ //End Watering
    digitalWrite(z3Valve,LOW);
    ...
    lastWater = millis();
    if(CalFlow == true){
      tarFlow = (Calz1Flow + Calz2Flow + Calz3Flow)/3;
